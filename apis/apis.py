...
def get_data_for_state(locale_name):
    """
    A crude first API to get Covid case data for US states based on a name,
        abbreviation, or FIPS code. Made modular because I hope to swap out the
        data source for a more general one soon.
    """
...
def check_cases_for_state(abbr):
    ...
    if response.ok:
        return list(filter(
            lambda state: state['state'] == abbr, response.json()
        ))[0]
